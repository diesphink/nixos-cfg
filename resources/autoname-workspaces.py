...
def rename_workspaces(ipc):
    for workspace in ipc.get_tree().workspaces():
        ...
        # if len(icon_tuple) == 1:
        #     name_parts["icons"] = f"{icon_tuple[0]}  "
        # elif len(icon_tuple) == 2:
        #     name_parts["icons"] = f"{icon_tuple[0]} <sup>{icon_tuple[1]}</sup>"
        # elif len(icon_tuple) > 2:
        #     name_parts["icons"] = f"{icon_tuple[0]} <sup> </sup>"
        #     # name_parts["icons"] = " "
        # else:
        #     name_parts["icons"] = None
        if not icon_tuple:
            name_parts["icons"] = None
        elif name_parts['num'] == '2':
            name_parts["icons"] = "  "
        elif len(icon_tuple) == 1:
            name_parts["icons"] = f"{icon_tuple[0]}  "
        else:
            name_parts["icons"] = f"{icon_tuple[0]} +"
