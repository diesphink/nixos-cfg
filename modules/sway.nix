...
in {
  ...
  home.packages = with pkgs; [
    ...
    (pkgs.writers.writePython3Bin "sph-first-empty-workspace" {libraries = with pkgs.python3.pkgs; [i3ipc]; flakeIgnore=["E501"];} (builtins.readFile ../resources/first-empty-workspace.py))

    (pkgs.writers.writePython3Bin "sph-autoname-workspaces" {libraries = with pkgs.python3.pkgs; [i3ipc]; flakeIgnore=["E501"];} (builtins.readFile ../resources/autoname-workspaces.py))
...
#      colors.background      = "${color7}";
      ...
      # ===============
      window.commands = [
        { criteria = { title = "(?:Open|Save) (?:File|Folder|As)"; }; command = "resize set 600 400"; }
        ...
        # Toggle apps
        { criteria = { app_id = "org.keepassxc.KeePassXC"; }; command = "floating enable, move scratchpad, focus"; }
        { criteria = { app_id = "com.rafaelmardojai.Blanket"; }; command = "floating enable, move scratchpad, focus"; }
        { criteria = { app_id = "blueman-manager"; }; command = "floating enable, move scratchpad, focus"; }
        { criteria = { app_id = "pavucontrol"; }; command = "floating enable, move scratchpad, focus"; }
        { criteria = { class = "obsidian"; }; command = "floating enable, move scratchpad, focus"; }
        { criteria = { class = "Plexamp"; }; command = "floating enable, move scratchpad, focus"; }
      ...
      startup = [
        ...
        # Always 
        { always = true; command = "pkill kanshi; ${pkgs.kanshi}/bin/kanshi";}
        { always = true; command = "pkill swayrd; env RUST_BACKTRACE=1 RUST_LOG=swayr=debug swayrd &> /tmp/swayrd.log"; }
        { always = true; command = "pkill poweralertd; sleep 5; poweralertd -s -i 'line power'"; }
        { always = true; command = "nwg-drawer -r -ovl"; }
        { always = true; command = "pkill playerctl; playerctl -a metadata --format \'{{status}} {{title}}\' --follow | while read line; do pkill -RTMIN+5 waybar; done"; }
      ];
